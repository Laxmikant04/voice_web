...
describe('lazyCache', () => {
    test('result is returned', async () => {
        expect(lazy_cache_1.default(randomString(), async () => 23, 0)()).resolves.toBe(23);
    });
    test('f is called once', async () => {
        const f = jest.fn().mockReturnValue(23);
        const cachedF = lazy_cache_1.default(randomString(), f, 1000);
        await cachedF();
        await cachedF();
    ...
    test('f is called twice', async () => {
        const f = jest.fn().mockReturnValue(23);
        const cachedF = lazy_cache_1.default(randomString(), f, -1);
        await cachedF();
        await cachedF();
