...
function findGoal(goals, type, threshold) {
    const goal = goals.find(g => g.type == type && g.count == threshold);
    return goal ? goal.reached_at : null;
}
const ONE_DAY = 24 * 60 * 60 * 1000;
const daysBetween = (date1, date2) => Math.floor((date1.getTime() - date2.getTime()) / ONE_DAY);
const formatDate = (date) => date
    .toISOString()
    .slice(0, 19)
    .replace('T', ' ');
async function hasComputedGoals(client_id) {
    const [[client]] = await db.query('SELECT has_computed_goals FROM user_clients WHERE client_id = ?', [client_id]);
    return Boolean(client.has_computed_goals);
}
async function computeGoals(client_id) {
    if (await hasComputedGoals(client_id))
        return;
    ...
    await db.query('UPDATE user_clients SET has_computed_goals = true WHERE client_id = ?', [client_id]);
}
exports.computeGoals = computeGoals;
async function getGoals(client_id, locale) {
    await computeGoals(client_id);
    const localeId = locale ? await db_1.getLocaleId(locale) : null;
 ...
 */
async function checkGoalsAfterContribution(client_id, locale) {
    if (!(await hasComputedGoals(client_id)))
        return;
    const localeId = 'name' in locale ? await db_1.getLocaleId(locale.name) : locale.id;
    let [[reachedGoals], [[{ clips_count, votes_count }]], [[streak]],] = await Promise.all([
        db.query('SELECT * FROM reached_goals WHERE client_id = ? AND locale_id = ?', [client_id, localeId]),
        db.query(`
        SELECT 
          (
           SELECT COUNT(*)
    ...
    for (const type of ['streak', 'clips', 'votes']) {
        const maxCount = reachedGoals.reduce((max, row) => row.type == type && row.count > max ? row.count : max, 0);
        ...
        VALUES (?,     ?,    ?,         ?,         NOW()     )`, [threshold, type, client_id, localeId]);
    }
}
exports.checkGoalsAfterContribution = checkGoalsAfterContribution;
